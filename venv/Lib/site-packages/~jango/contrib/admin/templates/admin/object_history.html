  ...
  <div id="change-history" class="module">
    ...
    <nav class="paginator" aria-labelledby="pagination">
      <h2 id="pagination" class="visually-hidden">
        {% blocktranslate with name=opts.verbose_name %}Pagination {{ name }}
        entries{% endblocktranslate %}
      </h2>
      {% if pagination_required %}
      <ul>
        {% for i in page_range %} {% if i == action_list.paginator.ELLIPSIS %}
      ...
      {% endif %} {{ action_list.paginator.count }} {% blocktranslate count
      counter=action_list.paginator.count %}entry{% plural %}entries{%
      endblocktranslate %}
    </nav>
    {% else %}
    <p>
